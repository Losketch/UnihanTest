<div align="center">
 
# 安卓系统设备Unicode中日韩统一汉字区段显示测试（UnihanTest）
[![总下载](https://img.shields.io/github/downloads/Losketch/UnihanTest/total.svg?color=critical&label=总下载量&logo=data:image/png;base64,)](https://githubfast.com/Losketch/UnihanTest/releases/latest)
[![星标](https://img.shields.io/github/stars/Losketch/UnihanTest?label=星标数&color=critical&logo=data:image/png;base64,
)](https://githubfast.com/Losketch/UnihanTest)

</div>

### 如果你是普通用户请直接 [点这里](https://githubfast.com/Losketch/UnihanTest/releases) 或到 [releases](https://githubfast.com/Losketch/UnihanTest/releases) 界面 下载直接配置好的安卓应用
----
修复原版BUG
 
支持Unicode 15.1.0.0

长期支持更新！

## 注意
此版本与[原版](https://githubfast.com/lzcapp/UnihanTest)不能共存！
也就是和[原版](https://githubfast.com/lzcapp/UnihanTest)包名一致

此版本签名使用[MT管理器](https://mt2.cn/)**默认签名**!
